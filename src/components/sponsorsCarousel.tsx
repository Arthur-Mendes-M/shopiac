...
export default function SponsorsInfiniteCarousel() {
  ...
  return (
    <section className="container w-full bg-gradient-background py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-200px),transparent_100%)]">
          {Array.from({ length: 3 }).map((el, idx) => {
            return (
              <ul key={idx} aria-hidden={idx !== 0} className="flex items-center justify-center md:justify-start [&_li]:mx-8 [&_img]:max-w-none animate-infinite-horizontal-scroll">
                {logos.map((image) => (
                  <li key={image.alt}>
                    <img
                      src={image.src}
